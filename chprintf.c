...
#ifdef CHPRINTF_BINARY
	// create a binary string of '0's and '1's
	static char *ulong_binary(char *p, uint32_t value, uint32_t width)
	{
		for (int i = 31, k = 0; i >= 0 && k < MAX_FREQ_PRESCISION; i--, k++)
		{
...
static char *ulong_freq(char *p, uint32_t freq, uint32_t precision, uint8_t show_prefix)
{
  ...
  while (true) {
    ...
    */
    ...
    case 'q':
      ...
      p = ulong_freq(p, value.u, precision, 1);
      break;
    case 'p':
      value.u = va_arg(ap, uint32_t);
      p = ulong_freq(p, value.u, precision, 0);
      break;

		#ifdef CHPRINTF_BINARY
    	 	 case 'b':
    	 		 value.u = va_arg(ap, uint32_t);
    	 		 p = ulong_binary(p, value.u, width);
    	 		 break;
		#endif
